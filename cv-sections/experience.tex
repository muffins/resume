...
\cventry
{Security Engineer, Malware Analysis} % Job title
{Meta} % Organization
{Seattle, WA} % Location
{Nov. 2020 - Current} % Date(s)
{ % Description(s) of tasks/responsibilities
\begin{cvitems}
\item {RE Malware (Windows, iOS, Android, macOS), RE SDKs for Ads fraud and abuse, RE software for vulnerability assessments}
\item {JSB/Dynamic Analysis rearchitecture, upgrade, terraform + Chef deployments}
\item {tdcli, automation libraries and CLI for Threat Intel knowledge store interface}
\end{cvitems}
}
